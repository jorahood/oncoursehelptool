...
<!-- suppressComment: show the comment button? -->
  <xsl:param name="suppressComment" select="''"/> 
<!-- for a sakai install, linkUrl should be /portal/help/TOCDisplay/content.hlp?docId= -->
  <xsl:param name="linkUrl" select="'/helptool/docs'"/>
...
<!-- root element -->
  <xsl:template match="/">
    ...
    <xsl:if test="not(boolean($suppressComment))">
      <p>
        <form name="comment" method="post" action="https://oncoursehelp.iu.edu/comments.cgi" target="comment">
...
<!-- anchor elements -->
  ...
  <!-- Handle <a> elements not embedded in another <a> element -->
    <xsl:if test="not(ancestor::a)">
      <xsl:variable name="isInternalAnchor" select="starts-with(@href, '#')" />
      <xsl:element name="a">
        <xsl:if test="@href">
          <xsl:attribute name="href">
            <xsl:call-template name="makeLinkURL">
              <xsl:with-param name="url" select="@href" />
              <xsl:with-param name="isInternalAnchor" select="$isInternalAnchor" />
            </xsl:call-template>
          </xsl:attribute>
          <xsl:if test="not($isInternalAnchor)">
...
<!-- When dealing with an internal anchor we don't want to generate a
     full url because this will cause the page to be reloaded. For
     internals we just want the "#anchor" value stored in the @href
     attribute of this anchor tag. -->
    <xsl:param name="isInternalAnchor" />
    <xsl:choose>
      <xsl:when test="$isInternalAnchor">
